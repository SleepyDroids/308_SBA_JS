...
function getLearnerData(course, ag, submissions) {
  ...
  */
  ...
  // had an infinite loop for a second but I see what I did wrong 
  // I previously had an unnecessary break; after i++; 
  ...
  while (i < submissions.length) {
    ...
    // increments through the loop just like the previous for loop
    i++;
  }
  ...
  for (let i = 0; i < submissions.length; i++) {
    ...
    if (thisAssignment) {
      ...
      if (!(dueDate > currentDate)) {
        ...
        pocket[studentID].pointsPossible =
          pocket[studentID].pointsPossible + thisAssignment.points_possible;
